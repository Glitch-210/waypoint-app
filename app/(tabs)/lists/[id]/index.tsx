...
//       setLoading(true);
//       if (isConnected) {
//         const res = await authFetch(`/api/places?listId=${encodeURIComponent(id)}`);
//         if (res.ok) {
...
//         method: 'PATCH',
//         body: JSON.stringify({ placeId, inRoute: newInRoute }),
//       });
...
//           <TouchableOpacity onPress={handleToggleOffline} disabled={isDownloading} style={styles.offlineButton}>
//             <MaterialIcons
...
//             return (
//               <Map style={styles.map} mapStyle={`https://api.maptiler.com/maps/streets-v2/style.json?key=${process.env.EXPO_PUBLIC_MAPTILER_API_KEY}`}>
//                 <Camera initialViewState={{ center: mapCenter, zoom: 12 }} />
//                 {routeGeometry && (
//                   <GeoJSONSource id="routeSource" data={routeGeometry as any}>
//                     <Layer id="routeLine" type="line" style={{ lineColor: colors.rausch, lineWidth: 4, lineJoin: 'round', lineCap: 'round' }} />
//                   </GeoJSONSource>
//                 )}
...
//
//       {canEdit && (
//         <TouchableOpacity style={styles.fab} onPress={() => router.push(`/lists/${id}/add`)}>
...
//   header: { flexDirection: 'row', alignItems: 'center', paddingTop: 50, paddingBottom: 16, paddingHorizontal: 16, backgroundColor: colors.canvas, borderBottomWidth: 1, borderBottomColor: colors.hairline, zIndex: 10 },
//   backButton: { marginRight: 16 },
//   title: { flex: 1, fontSize: 20, fontFamily: typography.bold, color: colors.ink },
//   avatarStack: { flexDirection: 'row', alignItems: 'center', marginLeft: 8 },
//   avatarCircle: { width: 28, height: 28, borderRadius: 14, backgroundColor: colors.rausch, justifyContent: 'center', alignItems: 'center', marginLeft: -8, borderWidth: 2, borderColor: colors.canvas },
//   avatarText: { fontSize: 10, color: colors.canvas, fontFamily: typography.bold },
//   offlineButton: { marginLeft: 8, padding: 4 },
//   offlineBanner: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#555', paddingVertical: 6, paddingHorizontal: 16 },
//   offlineBannerText: { color: colors.canvas, fontFamily: typography.medium, fontSize: 12 },
//   mapContainer: { flex: 0.4, backgroundColor: '#e0e0e0' },
//   map: { flex: 1 },
//   listContainer: { flex: 0.6 },
//   centerContainer: { flex: 1, justifyContent: 'center', alignItems: 'center' },
//   errorText: { color: colors.primary, fontFamily: typography.medium },
//   emptyCard: { backgroundColor: colors.canvas, borderRadius: 16, padding: 24, alignItems: 'center', marginHorizontal: 24, borderWidth: 1, borderColor: colors.hairline, gap: 8 },
//   emptyText: { fontSize: 18, color: colors.ink, fontFamily: typography.bold, marginTop: 4 },
//   emptySubText: { fontSize: 14, color: colors.muted, fontFamily: typography.regular, textAlign: 'center', lineHeight: 20 },
//   listContent: { padding: 16 },
//   fab: { position: 'absolute', right: 24, bottom: 24, width: 56, height: 56, borderRadius: 28, backgroundColor: colors.rausch, justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 4, elevation: 8 },
//   marker: { width: 24, height: 24, borderRadius: 12, justifyContent: 'center', alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 2, elevation: 4, borderWidth: 2, borderColor: '#FFF' },
//   markerRoute: { backgroundColor: colors.rausch },
//   markerInactive: { backgroundColor: colors.muted },
//   markerText: { color: '#FFF', fontSize: 12, fontFamily: typography.bold },
...
export default function ListDetailScreen() {
  return (
    <View style={styles.container}>
      <Text style={styles.text}>List Detail — stub (sign-in focus mode)</Text>
    </View>
  );
}
