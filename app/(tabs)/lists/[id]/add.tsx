...
//         if (data.place) { addPlace(data.place as any); }
//       }
//       router.back();
//     } catch (err) {
...
//                 placeholder="https://..."
//                 value={linkUrl}
//                 onChangeText={(text) => { setLinkUrl(text); if (parsedCandidate) setParsedCandidate(null); }}
//                 autoCapitalize="none"
//               />
//               {isParsing && (<ActivityIndicator size="small" color={colors.rausch} style={{ marginRight: 8 }} />)}
...
//             onCancel={() => { setParsedCandidate(null); setLinkUrl(''); }}
//           />
//         ) : (
//           <View style={styles.section}>
...
//                 value={query}
//                 onChangeText={(text) => { setQuery(text); if (selectedFeature) setSelectedFeature(null); }}
//               />
//               {query.length > 0 && (
...
//               )}
//             </View>
//             {isSearching && (<ActivityIndicator style={{ marginTop: 16 }} color={colors.rausch} />)}
//             {!selectedFeature && results.length > 0 && (
//               <View style={styles.resultsContainer}>
//                 {results.map((item) => (
//                   <TouchableOpacity key={item.id} style={styles.resultItem} onPress={() => handleSelectPlace(item)}>
...
//                 <TextInput style={[styles.input, styles.textArea]} value={notes} onChangeText={setNotes} placeholder="Why do you want to go here?" multiline numberOfLines={4} />
//               </View>
//             )}
//           </View>
//         )}
...
//   dividerText: { marginHorizontal: 12, fontSize: 13, fontFamily: typography.medium, color: colors.muted },
//   label: { fontSize: 14, fontFamily: typography.semibold, color: colors.ink, marginBottom: 8 },
//   searchContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.surfaceSoft, borderRadius: 8, paddingHorizontal: 12, borderWidth: 1, borderColor: colors.hairline },
//   searchIcon: { marginRight: 8 },
//   searchInput: { flex: 1, height: 48, fontSize: 16, fontFamily: typography.regular, color: colors.ink },
//   clearIcon: { padding: 8 },
//   resultsContainer: { marginTop: 8, backgroundColor: colors.canvas, borderRadius: 8, borderWidth: 1, borderColor: colors.hairline, overflow: 'hidden' },
//   resultItem: { flexDirection: 'row', alignItems: 'center', padding: 12, borderBottomWidth: 1, borderBottomColor: colors.surfaceStrong },
//   resultIcon: { marginRight: 12 },
//   resultText: { fontSize: 15, fontFamily: typography.medium, color: colors.ink },
//   resultSubText: { fontSize: 13, fontFamily: typography.regular, color: colors.muted },
//   formContainer: { marginTop: 16 },
//   input: { backgroundColor: colors.surfaceSoft, borderWidth: 1, borderColor: colors.hairline, borderRadius: 8, padding: 12, fontSize: 16, fontFamily: typography.regular, color: colors.ink },
//   textArea: { height: 100, textAlignVertical: 'top' },
//   footer: { padding: 16, borderTopWidth: 1, borderTopColor: colors.hairline },
//   saveButton: { backgroundColor: colors.rausch, borderRadius: 8, height: 48, justifyContent: 'center', alignItems: 'center' },
//   saveButtonDisabled: { backgroundColor: colors.rauschDisabled },
//   saveButtonText: { color: colors.canvas, fontSize: 16, fontFamily: typography.semibold },
// });
...
export default function AddPlaceScreen() {
  return (
    <View style={styles.container}>
      <Text style={styles.text}>Add Place — stub (sign-in focus mode)</Text>
    </View>
  );
}
